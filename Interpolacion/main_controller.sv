		...
		if (!rst_n) begin
			...
			case (state)
				...
				UPDATE_ADDR: begin
					...
					logic [8:0] next_x;
					next_x = x_cnt + (i_mode ? 9'd4 : 9'd1); // +4 en SIMD, +1 en Sequential
					
					if (next_x >= i_width) begin
						x_cnt <= '0;
		...
		case (state) 
			...
			WRITE_RES: begin
				...
				if (!i_mode) begin
					...
					byte_offset = (x_cnt/2) % 4;
					
					o_mem_byte_en = (4'b0001 << byte_offset); // Activa solo un byte
